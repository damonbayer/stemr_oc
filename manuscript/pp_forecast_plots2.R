...
deaths_forecast_plot <-
  ...
  # scale_color_brewer(name = "Credibility", guide = guide_legend(reverse = TRUE)) +
  scale_fill_brewer(name = "Credibility", guide = guide_legend(reverse = TRUE)) +
  theme_minimal_grid() +
...
pos_forecast_plot <-
  ...
  geom_lineribbon(
    ...
    color = "steelblue4",
    show.legend = T) +
  geom_point(size = 0.5, data = all_dat %>%
               filter(name == "pos"),
             mapping = aes(date, value)) +
...
pp_forecasts_plot <- plot_grid(deaths_forecast_plot, pos_forecast_plot, align = "hv") +
  patchwork::plot_annotation("Posterior Predictive Distributions & Probabilistic Forecasts")

save_plot(pp_forecasts_plot,
          filename = "~/Documents/oc_covid19_stemr_manuscript/figures/pp_forecast_plot.pdf",
          nrow = 3,
          ncol = 2,
          base_asp = 2.1,
          base_height = 2
