...
pp_data2 <- imap_dfr(multi_chain_stem_fit$stem_fit_list,  ~{
  ...
  select(time, tests, cases, deaths) %>%
  mutate(pos = cases / tests) %>%
  select(time, deaths, pos) %>%
  pivot_longer(-time) %>%
  group_by(time, name) %>%
...
handmade_plot <- ggplot() +
  geom_lineribbon(data = pp_data2, mapping = aes(time, value, ymin = .lower, ymax = .upper)) +
  geom_point(data = dat %>%
               mutate(pos = cases / tests) %>%
               select(time, deaths, pos) %>%
               pivot_longer(-time), mapping = aes(time, value)) +
...
# hmm, what happens when you pick a posterior sample and use simulate_stem to re-solve the ODEs. do you get the same latent path as the posterior?
...
a <- simulate_stem(stem_object = stem_object,
              nsim = 1,
              tparam_draws = list(stemr_tparams[[1]]), simulation_parameters = list(parameters),
              method = "ode", paths = F)$paths[[1]]
