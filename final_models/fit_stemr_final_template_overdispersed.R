...
my_seed <- as.numeric(args[2])

potential_args <- c("2020-09-13", "2020-09-20", "2020-09-27", "2020-10-04", "2020-10-11",
                    "2020-10-18", "2020-10-25", "2020-11-01", "2020-11-08", "2020-11-15",
                    "2020-11-22", "2020-11-29", "2020-12-06", "2020-12-13", "2020-12-20",
                    "2020-12-27", "2021-01-03", "2021-01-10", "2021-01-17", "2021-01-24",
                    "2021-01-31", "2021-02-07", "2021-02-14", "2021-02-21", "2021-02-28",
                    "double_initial_counts", "higher_ifr", "huntington_beach", "irvine",
                    "longer_infectious_period", "lower_alpha", "lower_R0", "santa_ana"
)

stopifnot("args[1] is not in potential_args" = args[1] %in% potential_args)
...
if (Sys.info()[["sysname"]] == "Linux") {
  if (args[1] %in% c("irvine", "santa_ana", "huntington_beach")) {
    oc_data <- read_csv("/data/homezvol2/bayerd/uci_covid_modeling2/data/oc_city_data.csv") %>%
      filter(city == case_when(args[1] == "irvine" ~ "Irvine",
                               args[1] == "santa_ana" ~ "Santa Ana",
                               args[1] == "huntington_beach" ~ "Huntington Beach")) %>%
      select(-city)
  } else {
    oc_data <- read_csv("/data/homezvol2/bayerd/uci_covid_modeling2/data/oc_data.csv")
  }
  death_delay_ecdf <- read_rds("/data/homezvol2/bayerd/uci_covid_modeling2/data/death_delay_ecdf.rds")
} else if (Sys.info()[["sysname"]] == "Darwin") {
  if (args[1] %in% c("irvine", "santa_ana", "huntington_beach")) {
    oc_data <- read_csv("~/Documents/uci_covid_modeling2/data/oc_city_data.csv") %>%
      filter(city == case_when(args[1] == "irvine" ~ "Irvine",
                               args[1] == "santa_ana" ~ "Santa Ana",
                               args[1] == "huntington_beach" ~ "Huntington Beach")) %>%
      select(-city)
...
if (args[1] %in% c("2020-08-16", "2020-09-13", "2020-10-11", "2020-11-08", "2020-12-06",
                   "2021-01-03", "2021-01-31", "2021-02-28")) {
  last_day <- ymd(args[1])
} else {
...
to_estimation_scale = function(params_nat) {
  c(R0_init_est = log(params_nat[["R0_init"]]), # log(R0_init)
    ...
    phi_death_est = log(params_nat[["phi_death"]]), # -0.5 * log(phi_death)
    alpha_init_est = log(params_nat[["alpha_init"]]), # log(alpha)
    # kappa_est = -0.5 * log(params_nat[["kappa"]]), # -0.5 * log(kappa)
    kappa_est = log(params_nat[["kappa"]]), # -0.5 * log(kappa)
...
phi_death_est_mean <- 5.4
phi_death_est_sd <- 0.46
...
logprior =
  function(params_est) {
    sum(
        ...
        dnorm(params_est["phi_death_est"], mean = phi_death_est_mean, sd = phi_death_est_sd, log = TRUE), # log(phi)
        ...
        dnorm(params_est["kappa_est"], mean = kappa_est_mean, sd = kappa_est_sd, log = TRUE), # log(kappa)
...
initial_values <- structure(c(1.15122629457544, 1.25649794322914, 1.25854844884716, 
                              ...
                              4.353397657496, 1119.16877460235, 1157.88515888053, 1216.59780728499, 
                              1253.80180270737, 1112.47357858078, 1066.87190444604, 858.392279262288, 
                              1021.17345142097, 1141.79402038238, 1541.89792896947, 0.00284695022744819, 
                              ...
                              0.0672260859059286, 0.0588415162180674), .Dim = 10:11, .Dimnames = list(
                                NULL, c("R0_init", "gamma", "dur_infec", "rho_death", "phi_death", 
                                        "alpha_init", "kappa", "ifr_init", "sigma_R0", "sigma_ifr", 
                                        "sigma_alpha")))[my_seed, ]
...
write_rds(args, paste0("/dfs6/pub/bayerd/res_fit_stemr_final_models_overdispersion_", args[1], "_", my_seed, "_", format(Sys.time(), "%Y_%m_%d_%H_%M_%S"), ".rds"))
