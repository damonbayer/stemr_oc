library(tidyverse)
enbracket <- function(string) str_c("[", string, "]")
print_float <- function(number) scales::number(number, accuracy = 0.1)
dat <- read_rds("/Users/damon/Documents/stemr_oc/final_models/data/dat.rds")
cat(str_c("tmax = ", length(dat$cases)))
cat("tspan = (0.0, tmax)")
cat("obstimes = 1.0:1.0:tmax")
cat("const popsize = 3175692.0")
cat(str_c("const tests = ", enbracket(str_c(dat$tests, collapse = ", "))))
str_c("const param_change_times = collect(1.0:1.0:", print_float(length(dat$cases) - 1), ")")
str_c("const seroprev_times = ", enbracket(print_float(dat_seroprev$time)))
str_c("const seroprev_tests = ", enbracket(dat_seroprev$seroprev_tests))
str_c("data_seroprev_cases = ", enbracket(dat_seroprev$seroprev_cases))


cat(str_c("tmax = ", print_float(length(dat$cases))),
    "tspan = (0.0, tmax)",
    "obstimes = 1.0:1.0:tmax",
    "const popsize = 3175692.0",
    str_c("const tests = ", enbracket(str_c(dat$tests, collapse = ", "))),
    str_c("const param_change_times = collect(1.0:1.0:", print_float(length(dat$cases) - 1), ")"),
    str_c("const seroprev_times = ", enbracket(print_float(dat_seroprev$time))),
    str_c("const seroprev_tests = ", enbracket(dat_seroprev$seroprev_tests)),
    str_c("data_seroprev_cases = ", enbracket(dat_seroprev$seroprev_cases)),
    str_c("data_new_deaths = ", enbracket(str_c(dat$deaths, collapse = ", "))),
    str_c("data_new_cases = ", enbracket(str_c(dat$cases, collapse = ", "))),
    sep = "\n")
...
tparam_draws <- list(
  ...
  c(
    -0.0858908049474203, -0.0858908049474203, -0.0858908049474203, 0.549281683468169,
...
# β_t_values
dput(exp(c(log(simulation_parameters[["R0_init"]]), log(simulation_parameters[["R0_init"]]) + head(cumsum(tparam_draws[[1]]), -1) * simulation_parameters[["sigma_R0"]]) - log_popsize - log(simulation_parameters[["dur_infec"]])) * popsize)

# IFR_t_values
dput(c(simulation_parameters[["ifr_init"]], expit(logit(simulation_parameters[["ifr_init"]]) + head(cumsum(tparam_draws[[2]]), -1) * simulation_parameters[["sigma_ifr"]])))

# α_t_values
dput(c(simulation_parameters[["alpha_init"]], exp(log(simulation_parameters[["alpha_init"]]) + head(cumsum(tparam_draws[[3]]), -1) * simulation_parameters[["sigma_alpha"]])))

# γ
dput(simulation_parameters[["gamma"]])

# ν
dput(1 / simulation_parameters[["dur_infec"]])

# u0
init_dist[c("S_0", "E_0", "I_0", "R_0", "D_0", "I_0")] %>% cat(sep = ", ")

# ρ_death
dput(simulation_parameters[["rho_death"]])

simulation_parameters[["phi_death"]]

simulation_parameters[["kappa"]]
